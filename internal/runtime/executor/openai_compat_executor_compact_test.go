...
func TestOpenAICompatExecutorCopilotResponsesUsesResponsesEndpoint(t *testing.T) {
	var gotPath string
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
	...
	payload := []byte(`{"model":"gpt-5.3-codex","input":"hi","stream":false,"max_output_tokens":16}`)
	resp, err := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		Model:   "gpt-5.3-codex",
		Payload: payload,
	}, cliproxyexecutor.Options{
...
func TestOpenAICompatExecutorPreservesReasoningContentBeforeUpstream(t *testing.T) {
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
		gotBody = body
	...
	if got := gjson.GetBytes(gotBody, "messages.2.reasoning_content").String(); got != "real reasoning" {
		t.Fatalf("messages.2.reasoning_content = %q, want real reasoning; body=%s", got, string(gotBody))
	}
}
