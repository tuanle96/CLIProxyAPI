...
func TestConvertOpenAIRequestToAntigravity_VideoBase64InImageUrl(t *testing.T) {
	inputJSON := []byte(`{
	...
	output := ConvertOpenAIRequestToAntigravity("gemini-3.5-flash", inputJSON, false)
	...
	if got := inlineData.Get("mimeType").String(); got != "video/mp4" {
		t.Errorf("Expected mimeType 'video/mp4', got '%s'", got)
	}
...
func TestConvertOpenAIRequestToAntigravity_VideoBase64InVideoUrlObject(t *testing.T) {
	inputJSON := []byte(`{
		"model": "gemini-3.5-flash",
		"messages": [
			{
	...
	if got := inlineData.Get("mimeType").String(); got != "video/webm" {
		t.Errorf("Expected mimeType 'video/webm', got '%s'", got)
	}
...
func TestConvertOpenAIRequestToAntigravity_VideoBase64InVideoUrlString(t *testing.T) {
	inputJSON := []byte(`{
		"model": "gemini-3.5-flash",
		"messages": [
			{
	...
	if got := inlineData.Get("mimeType").String(); got != "video/quicktime" {
		t.Errorf("Expected mimeType 'video/quicktime', got '%s'", got)
	}
...
func TestConvertOpenAIRequestToAntigravity_VideoBase64IgnoresMalformedURLsAndParsesParams(t *testing.T) {
	inputJSON := []byte(`{
		"model": "gemini-3.5-flash",
		"messages": [
			{
	...
	parts := gjson.GetBytes(output, "request.contents.0.parts").Array()
	if len(parts) != 2 {
		t.Fatalf("Expected only text plus the valid inline video, got %d parts: %s", len(parts), gjson.GetBytes(output, "request.contents.0.parts").Raw)
	}
